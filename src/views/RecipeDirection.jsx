            ...
            <div>
              <div className="h-64 md:h-2/4">
                <div className='flex justify-between flex-wrap'>
                  <h1 className="text-2xl text-center font-medium">How to cook {directions.name}</h1>
                  <p className='bg-lemon-meringue  p-2  rounded-2xl'>{directions.time} min</p>
                </div>
                <div className='h-64 justify-center mx-auto text-center'>
                  <p>{remainingTime > 0 ? `${remainingTime} seconds` : 'Time up!'}</p>
                  <img src={directions.imageURL} alt={directions.name} className="rounded-full h-full w-64 mx-auto object-cover" />
                </div>
              </div>
              <div className="mt-28">
                ...
                <div className="flex justify-between  mt-4">
                  {timer ? (
                    <button onClick={pauseTimer} className='border-black border w-10 p-2 rounded-2xl bg-copper-orange'>
                        <img src={pause} alt="" />
                    </button>
                  ) : (
                    <button className='border-black border p-2 rounded-2xl  bg-copper-orange' onClick={() => startTimer(directions.steps[currentStep].time)}>
                        <img src="" alt="" />
                    </button>
                  )}
                  <button className='border-black border p-2  w-10 rounded-2xl bg-copper-orange' onClick={moveToPreviousStep} disabled={currentStep === 0}>
                    <img src={leftArrow} alt="" />
                  </button>
                  <button className='border-black border p-2  w-10 rounded-2xl bg-copper-orange' onClick={moveToNextStep} disabled={remainingTime === ''}>
